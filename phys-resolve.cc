...
{
  SectTT sect = box_sphere_interpenetrate(box,ball->position,ball->radius, opt_sticky);
  if ( !sect.exists ) 
    return false;
...
//Edit
bool
World::sticky_box_ball_resolve(Box *box, Ball *ball)
{
  SectTT sect = box_sphere_interpenetrate(box,ball->position,ball->radius, opt_sticky);
  if ( !sect.exists )
    return false;
  ...
  if (opt_move_item == MI_Ball) {
    box_sticky = box;
    ball->position = box->position;
  }
  if (opt_move_item == MI_Ball_V) {
    box->velocity = ball->velocity;
  }
  return true;
