...
World::setup_tower(float base, int layers)
{
  ...
  const bool balls = false;
  bool block_rotate = true; // sets if block rotates 90 degrees
  ...
  for ( int i=0; i<11; i++ )
      ...
      for ( int j=0; j<3; j++ ) 
	{
	  ...
	  if (block_rotate) {
	    ...
	    // offset of block due to rotating at the middle of blocks - z dirrection
	    b->translate(pVect(block_space,0,-block_space));
	    
	    // rotate
	    b->rotate(pVect(0,1,0),M_PI / 2);
	  }
	  else {
	  ...
	  //--set texture--//
	  //	  for ( int k=0; i<6; k++ )
	  //	    b->set_face_texture(k,texid_mural);
	  ...
	  //	  if ( glued && i == 0 ) b->set_read_only(); 
          ...
          // 3 blocks created. Increment the y position for next layer.
	  block_base += pVect(0,block_diag.y,0);
	  
	  // alternate rotation
	  block_rotate = not block_rotate;
